from base.services import get_milvus
...
def test_milvus_db():
  dimensions = 8
  log("start connecting to Milvus")
  client = get_milvus()
  has = client.has_collection("hello_milvus")
  log(f"Does collection hello_milvus exist in Milvus: {has}")
  #################################################################################
  # 2. create collection
  ...
  log(fmt.format("Create collection `hello_milvus`"))
  ...
  log(f"Number of entities in Milvus: {insert_result['insert_count']}")  # check the num_entities

  ################################################################################
  # 4. create index
  # We are going to create an IVF_FLAT index for hello_milvus collection.
  # create_index() can only be applied to `FloatVector` and `BinaryVector` fields.
  log(fmt.format("Start Creating index IVF_FLAT"))
  ...
  for hits in result:
      for hit in hits:
          log(f"hit: {hit}, random field: {hit.get('entity').get('random')}")
  log(search_latency_fmt.format(end_time - start_time))

  # -----------------------------------------------------------------------------
  # query based on scalar filtering(boolean, int, etc.)
  log(fmt.format("Start querying with `random > 0.5`"))
  ...
  log(f"query pagination(limit=4):\n\t{r1}")
  log(f"query pagination(offset=1, limit=3):\n\t{r2}")


  # -----------------------------------------------------------------------------
  # filtered search
  log(fmt.format("Start filtered searching with `random > 0.5`"))
  ...
  log(fmt.format(f"Start deleting with expr `{expr}`"))
  ...
  log(f"query before delete by expr=`{expr}` -> result: \n-{result[0]}\n-{result[1]}\n")
  ...
  log(f"query after delete by expr=`{expr}` -> result: {result}\n")

  ###############################################################################
  # 7. drop collection
  # Finally, drop the hello_milvus collection
  log(fmt.format("Drop collection `hello_milvus`"))
